...
export default async function Page(props: {
    ...
    const activeStudentsCount = await db
        ...
        .from(userTable)
        .where(
            and(eq(userTable.status, "active"), eq(userTable.role, "student")),
        );
    const filteredStudentsCount = await db
        .select({ count: count() })
        .from(userTable)
        .where(
            query
        ...
        <>
                    ...
                    badges={
                        <>
                            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 font-bold">
                                <FileText size={15} />
                                {activeStudentsCount[0].count} Active Classes
                            </span>
                            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 font-bold">
                                <Check size={15} />
                                {totalStudentsCount[0].count} Total
                            </span>
                        </>
                    }
                    actionButton={
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-surface-dark rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                    ...
                    {filteredStudentsCount[0].count > 0 && (
                        <Pagination
                            filteredContentCount={
                                filteredStudentsCount[0].count
                            }
