...
import { classApplicationTable } from "@/db/schema";
...
import Table from "@/components/admin/class-applications/table";
...
export default async function Page(props: {
    ...
    const totalClassApplicationsCount = await db
        .select({ count: count() })
        .from(classApplicationTable);
    const submittedApplicationsCount = await db
        .select({ count: count() })
        .from(classApplicationTable)
        .where(eq(classApplicationTable.status, "submitted"));
    const filteredClassApplicationsCount = await db
        .select({ count: count() })
        .from(classApplicationTable)
                    ...
                    badges={
                        <>
                            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 font-bold">
                                <FileText size={15} />
                                {submittedApplicationsCount[0].count} Submitted
                                Applications
                            </span>
                            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 font-bold">
                                <Check size={15} />
                                {totalClassApplicationsCount[0].count} Total
                            </span>
                        </>
                    }
                    actionButton={
                ...
                <div className="bg-white dark:bg-surface-dark rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                    ...
                    <Pagination
                        filteredContentCount={
                            filteredClassApplicationsCount[0].count
                        }
